...
var submitImage = function submitImage(req, res, file) {
    return new Promise(function (resolve, reject) {
        ...
        db.connect().then(function (conn) {
            r.table("Images").insert(img).run(conn).then(function (dbres) {
...
var changeStatus = function changeStatus(imgid, status) {
    return new Promise(function (resolve, reject) {
        db.connect().then(function (conn) {
            r.table("Images").get(imgid).update({ status: status }).run(conn).then(function (dbres) {
...
var approveImage = function approveImage(imgid) {
    return changeStatus(imgid, 1);
};

var rejectImage = function rejectImage(imgid) {
