...
var submitMessage = function submitMessage(req, res, msgtxt) {
    return new Promise(function (resolve, reject) {
        ...
        db.connect().then(function (conn) {
            r.table("Messages").insert(msg).run(conn).then(function (dbres) {
...
var changeStatus = function changeStatus(msgid, status) {
    return new Promise(function (resolve, reject) {
        db.connect().then(function (conn) {
            r.table("Messages").get(msgid).update({ status: status }).run(conn).then(function (dbres) {
...
var approveMessage = function approveMessage(msgid) {
    return changeStatus(msgid, 1);
};

var rejectMessage = function rejectMessage(msgid) {
    return changeStatus(msgid, 2);
};

var processMessageChanges = function processMessageChanges(state, changes) {
